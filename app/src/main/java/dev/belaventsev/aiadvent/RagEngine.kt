...
class RagEngine(
    ...
    suspend fun askWithRag(
        ...
        val systemPrompt = """
            ...
            |2. Затем ОБЯЗАТЕЛЬНО приведи 1-2 цитаты из контекста в кавычках «», которые подтверждают твой ответ.
            |   Цитаты — это дословные фрагменты из контекста, без изменений.
            |
            |ПРИМЕР ОТВЕТА:
            |Стоимость проезда составляет 100 рублей в одну сторону.
            |
            |Цитата: «Стоимость проезда в одну сторону 100 рублей, туда-обратно 200 рублей.»
            |
            |ПРАВИЛА:
            |- Используй ТОЛЬКО факты из контекста. Не добавляй ничего от себя.
            |- Ответ БЕЗ цитаты — это ОШИБКА. Всегда включай хотя бы одну цитату.
            |- Если в контексте нет информации для ответа — напиши:
            |  "Не могу ответить на основе имеющихся документов. Попробуйте уточнить вопрос."
